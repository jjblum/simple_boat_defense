...
def splineToEuclidean2D(length, sx, sy, s):
    ...
    :return: numpy array [x, y], with shape (1,2)
    """
    return np.atleast_2d(np.array([np.interp(s, length, sx), np.interp(s, length, sy)]))
...
def closestPointOnSpline2D(length, sx, sy, x, y, guess=None):
    ...
    :return: s* from interval [0, total length], point closest to input (x,y)
    """
    s_star = 0.0
    N = len(sx)
    ...
    if guess is None:
        ...
        if min_guess_halfwidth <= closest <= N - min_guess_halfwidth:
            ...
            s_quad[0] = length[N - 2*min_guess_halfwidth]
            s_quad[1] = length[N - min_guess_halfwidth]
            s_quad[2] = length[N]
    ...
    for iteration in range(4):
        ...
        for q in range(3):
            D_quad[q] = spatial.distance.sqeuclidean(X, splineToEuclidean2D(length, sx, sy, s_quad[q]))
    ...
    return s_star, result[0][0], result[0][1]
...
if __name__ == "__main__":
    ...
    test_x = 1.
    test_y = 9.
    s_star, result_x, result_y = closestPointOnSpline2D(length, sx, sy, test_x, test_y, guess=None)
    plt.plot(test_x, test_y, 'r+', markersize=12., markeredgewidth=2.0)
    result = splineToEuclidean2D(length, sx, sy, s_star)
    plt.plot(result_x, result_y, 'gx', markersize=12., markeredgewidth=2.0)
    result_line = np.array([[test_x, test_y], [result_x, result_y]])
    plt.plot(result_line[:, 0], result_line[:, 1], 'b-')
    plt.axis('equal')
    plt.title("length = {:.2f}, s = {}".format(total_length, s_star))
    plt.show()
